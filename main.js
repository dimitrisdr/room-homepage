...
const descriptionTitles = [
    ...
    `Discover innovative ways to decorate`, `We are available all across the globe`, `Manufactured with the best materials`
...
let currentIndex = 0;
let descriptionTitleEl = document.querySelector('.description-section .main-title');
...
let mainPictureSourceTag = document.querySelector('.header__img-container source');
...
moveBtns.forEach(btn => btn.addEventListener('click', ()=> {
    ...
    currentIndex  = (currentIndex === 0 && direction === -1 )? elementsMaxIndex : (currentIndex + direction) % 3;
    ...
    mainImageTag.setAttribute('src', `images/mobile-image-hero-${currentIndex + 1}.jpg`)
    mainPictureSourceTag.setAttribute('srcset', `images/desktop-image-hero-${currentIndex + 1}.jpg`)
